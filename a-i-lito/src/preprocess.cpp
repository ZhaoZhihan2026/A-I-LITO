...
Preprocess::Preprocess() : feature_enabled(0), lidar_type(AVIA), blind(0.01), point_filter_num(1), adaptive_point_filter_num(1), adaptive_filter_size_surf(0.1), adaptive_vio_max_iterations(5)
{
  inf_bound = 10;
  N_SCANS = 6;
  group_size = 8;
...
void Preprocess::avia_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg)
{
  ...
  uint valid_num = 0;
  // // ====================== zzh新增：场景判断与自适应阈值计算 ======================
  // // 1. 参数设定（可放入配置文件，此处硬编码为示例）
  // const float R_near = 5.0f;       // 近区半径 (m)
  // const float R_far  = 15.0f;      // 远区半径 (m)
  // const float theta_open = 3.0f;       // 开阔度阈值
  // const float dist_th_base = 150.0f; // 基础距离阈值
  // const float dist_min = 30.0f;    // 最小距离阈值
  // const float dist_max = 200.0f;   // 最大距离阈值

  // // 2. 统计近/远区点数量（仅遍历一次，O(N)复杂度）
  // int N_near = 0, N_far = 0;
  // for (uint i = 0; i < plsize; i++)
  // {
  //   if (msg->points[i].line >= N_SCANS) continue; // 过滤无效线号
    
  //   // 计算点到LiDAR中心的距离平方
  //   float x = msg->points[i].x;
  //   float y = msg->points[i].y;
  //   float z = msg->points[i].z;
  //   float dist_sqr = x*x + y*y + z*z;
    
  //   // 统计近区和远区内的点
  //   if (dist_sqr <= R_near * R_near) N_near++;
  //   if (dist_sqr <= R_far  * R_far)  N_far++;
  // }

  // // 3. 计算开阔度指标与自适应距离阈值
  // float open_ratio = static_cast<float>(N_far) / std::max(N_near, 1); // 避免除零
  // float dist_th = dist_th_base * (open_ratio / theta_open);
  // dist_th = std::clamp(dist_th, dist_min, dist_max); // 限幅到合理范围


  // // 输出调试信息（可选）
  // printf("[ Adaptive ] open_ratio=%.2f, dist_th=%.1fm, scene=%s\n",
  //        open_ratio, dist_th, 
  //        open_ratio > theta_open ? "开阔" : "狭窄");
  // // ==========================================================================
  ...
  else
  {
    for (uint i = 0; i < plsize; i++)
    {
      if ((msg->points[i].line < N_SCANS)) // && ((msg->points[i].tag & 0x30) == 0x10))
      {
        ...
        {
          double distance_sqr = pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y + pl_full[i].z * pl_full[i].z;
          double intensity = pl_full[i].intensity;//zzhi
          if (distance_sqr >= blind_sqr && intensity >= 15 && intensity <= 240) //zzhi
          // if (distance_sqr >= blind_sqr && distance_sqr <= dist_th * dist_th && intensity >= 15) //zzhi
...
void Preprocess::velodyne_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  // ====================== zzh连续动态参数调整 ======================
  // 1. 场景感知参数
  const float R_inner   = 5.0f;        // 内层半径
  const float R_outer   = 20.0f;       // 外层半径
  const float inner_k   = 0.6f;        // inner_ratio -> α 的归一化系数
  const float span_max  = 30.0f;       // 最大距离归一化上限
  const float sig_shift = 0.55f;       // Sigmoid 中心点
  const float sig_k     = 8.0f;        // Sigmoid 陡峭度 8.0
  ...
  for (int i = 0; i < plsize; ++i) {
      ...
      float d = sqrtf(pt.x*pt.x + pt.y*pt.y + pt.z*pt.z);

      min_dist = std::min(min_dist, d);
      max_dist = std::max(max_dist, d);
      if (d <= R_inner) ++N_inner;
      if (d <= R_outer) ++N_outer;
  }

  // 3. 狭窄度 α ∈ [0,1]（0 开阔，1 狭窄）
  float inner_ratio = static_cast<float>(N_inner) / std::max(N_outer, 1);
  float alpha_raw   = std::clamp(inner_ratio / inner_k, 0.0f, 1.0f);
  alpha_raw         = std::max(alpha_raw, std::clamp(1.0f - max_dist / span_max, 0.0f, 1.0f));

  // Sigmoid 平滑
  float alpha = 1.0f / (1.0f + expf(-sig_k * (alpha_raw - sig_shift)));
  alpha       = std::clamp(alpha, 0.0f, 1.0f);

  // 4. 参数边界（安全值）
  const int   filter_num_min = 3,  filter_num_max = 5;
  const int   iter_min       = 4,  iter_max       = 5;
  const float surf_min       = 0.15f, surf_max   = 0.4f;


  int   adaptive_point_filter_num   = static_cast<int>(filter_num_min + alpha * (filter_num_max - filter_num_min) + 0.5f);
  int   adaptive_vio_max_iterations = static_cast<int>(iter_min       + alpha * (iter_max       - iter_min)       + 0.5f);
  float adaptive_filter_size_surf   = surf_min + alpha * (surf_max - surf_min);


  // 6. 更新全局
  point_filter_num = adaptive_point_filter_num;

  // 7. 调试日志
  ROS_INFO_STREAM("[Dynamic] α=" << std::fixed << std::setprecision(2) << alpha
                  << ", min=" << min_dist << "m, max=" << max_dist << "m"
                  << ", filter_num=" << adaptive_point_filter_num
                  << ", surf=" << adaptive_filter_size_surf
                  << ", iter=" << adaptive_vio_max_iterations);
  // =========================================================
  ...
  else
  {
    for (int i = 0; i < plsize; i++)
    {
      ...
      point_filter_num = adaptive_point_filter_num;//zzh自适应参数

      if (i % point_filter_num == 0)
      {
        double distance_sqr = added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z;//zzhi
        if  (distance_sqr >= blind_sqr && added_pt.intensity >= 15 && added_pt.intensity <= 240)//zzhi
        // if  (distance_sqr >= blind_sqr && distance_sqr <= dist_th*dist_th && added_pt.intensity >= 15 && added_pt.intensity <= 240)//zzhi
