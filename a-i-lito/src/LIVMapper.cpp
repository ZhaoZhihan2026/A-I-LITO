...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  max_iterations = p_pre->adaptive_vio_max_iterations;//zzh自适应参数
...
{
  filter_size_surf_min = p_pre->adaptive_filter_size_surf;//zzh自适应参数
...
cv::Mat LIVMapper::getImageFromMsg(const sensor_msgs::ImageConstPtr &img_msg)
{
  ...
  if (img_msg->encoding == "8UC3") {
    sensor_msgs::Image tmp_img;         // 创建临时ROS图像消息对象
    tmp_img.header = img_msg->header;   // 复制消息头（时间戳/坐标系）
    tmp_img.height = img_msg->height;   // 复制图像高度
    tmp_img.width = img_msg->width;     // 复制图像宽度
    tmp_img.is_bigendian = img_msg->is_bigendian; // 复制字节序标志
    tmp_img.step = img_msg->step;       // 复制每行字节数
    tmp_img.data = img_msg->data;       // 共享原始像素数据（无拷贝）
    tmp_img.encoding = "bgr8";          // 强制指定编码为BGR格式
    cv_bridge::CvImageConstPtr ptr = cv_bridge::toCvCopy(tmp_img, sensor_msgs::image_encodings::BGR8);// 执行带验证的格式转换
    img = ptr->image;  // 直接获取BGR彩色图像
  }
  else if (img_msg->encoding == "16UC1") {
    cv_bridge::CvImageConstPtr ptr1; 
    sensor_msgs::Image tmp_img;
    tmp_img.header = img_msg->header;
    tmp_img.height = img_msg->height;
    tmp_img.width = img_msg->width;
    tmp_img.is_bigendian = img_msg->is_bigendian;
    tmp_img.step = img_msg->step;
    tmp_img.data = img_msg->data;
    tmp_img.encoding = "mono16"; // 先转换为单通道16位格式
    cv_bridge::CvImageConstPtr ptr = cv_bridge::toCvCopy(tmp_img, sensor_msgs::image_encodings::MONO16);
    cv::Mat mono16_img = ptr->image;
    // 将16位单通道图像转换为8位单通道图像
    cv::Mat mono8_img;
    mono16_img.convertTo(mono8_img, CV_8U, 1.0 / 256.0); // 假设最大值为65535，缩放到0-255范围
    // 创建临时图像消息用于转换为BGR8
    sensor_msgs::Image mono8_msg;
    mono8_msg.header = img_msg->header;
    mono8_msg.height = mono8_img.rows;
    mono8_msg.width = mono8_img.cols;
    mono8_msg.is_bigendian = false;
    mono8_msg.step = mono8_img.cols;
    mono8_msg.data.assign(mono8_img.data, mono8_img.data + mono8_img.total());
    mono8_msg.encoding = "mono8";
    cv_bridge::CvImageConstPtr ptr_bgr = cv_bridge::toCvCopy(mono8_msg, sensor_msgs::image_encodings::BGR8);
    img = ptr_bgr->image;  
  }
  else {
...
void LIVMapper::img_cbk(const sensor_msgs::ImageConstPtr &msg_in)
{
  ...
  if (abs(msg_header_time - last_timestamp_img) < 0.1) return; //zzh 下采样控制频率
  ...
  // if (!img_cur.empty()) {
  //   cv::Mat img_float;
  //   img_cur.convertTo(img_float, CV_32F);
  //   cv::Mat reshaped = img_float.reshape(1, img_float.rows * img_float.cols);
    
  //   cv::Mat w, u, vt;
  //   cv::SVDecomp(reshaped, w, u, vt, cv::SVD::MODIFY_A | cv::SVD::FULL_UV);
  
  //   int k = std::max(10, static_cast<int>(w.rows * 0.1));
  //   k = std::min(k, w.rows);
  
  //   cv::Mat w_k = cv::Mat::zeros(w.rows, 1, CV_32F);
  //   for (int i = 0; i < k; i++) {
  //     w_k.at<float>(i) = w.at<float>(i);
  //   }
    
  //   cv::Mat denoised = u * cv::Mat::diag(w_k) * vt;
    
  //   denoised = denoised.reshape(1, img_float.rows);
    
  //   cv::normalize(denoised, img_cur, 0, 255, cv::NORM_MINMAX);
  //   img_cur.convertTo(img_cur, CV_8U);
  // }
  // // SVD去噪声处理zzh
