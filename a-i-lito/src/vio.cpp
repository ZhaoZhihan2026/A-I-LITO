...
#include "vio.h"
// zzh在文件顶部添加函数声明
float calculateGradientError(const cv::Mat& ref_patch, const cv::Mat& cur_patch);
float huber_loss(float x, float delta);
// zzh在文件顶部添加函数声明
...
void VIOManager::retrieveFromVisualSparseMap(cv::Mat img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &plane_map)
{
  ...
  // zzh打开文件用于写入梯度误差
  // std::ofstream gradient_error_file("/home/jing/gradient_errors.txt", std::ios::app); // 以追加模式打开文件
  ...
  for (int i = 0; i < length; i++)
  {
    if (grid_num[i] == TYPE_MAP)
    {
      ...
      // // zzh 计算参考图像块和当前图像块的梯度
      // // zzh计算像素误差
      // float pixel_error = 0.0;
      // for (int ind = 0; ind < patch_size_total; ind++) {
      //     pixel_error += (ref_ftr->inv_expo_time_ * patch_wrap[ind] - state->inv_expo_time * patch_buffer[ind]) *
      //                   (ref_ftr->inv_expo_time_ * patch_wrap[ind] - state->inv_expo_time * patch_buffer[ind]);
      // }

      // // zzh计算梯度误差
      // cv::Mat ref_patch(patch_size, patch_size, CV_32F, patch_wrap.data());
      // cv::Mat cur_patch(patch_size, patch_size, CV_32F, patch_buffer.data());
      // float gradient_error = calculateGradientError(ref_patch, cur_patch);
      // if (gradient_error > 1000) continue;

      // // zzh将梯度误差写入文件
      // gradient_error_file << gradient_error << std::endl;

      // error = pixel_error;

      // // 结合两种误差
      // float alpha = 0.7; // 梯度误差权重
      // float error = alpha * gradient_error + (1-alpha) * pixel_error;

      // // 使用提高后的异常值阈值
      // float outlier_threshold = 200.0;
      // if (error > outlier_threshold * patch_size_total) continue;
      // // zzh 计算参考图像块和当前图像块的梯度

      // zzh  NCC 值小于阈值 ncc_thre，则跳过该特征点 （原来有，没有开）
      if (ncc_en) // fasle
      {
  ...
  // gradient_error_file.close();  // zzh关闭文件
...
float calculateGradientError(const cv::Mat& ref_patch, const cv::Mat& cur_patch) {
  // 使用3x3 Sobel核计算梯度
  cv::Mat ref_grad_x, ref_grad_y, cur_grad_x, cur_grad_y;
  cv::Sobel(ref_patch, ref_grad_x, CV_32F, 1, 0, 3); // X方向梯度
  cv::Sobel(ref_patch, ref_grad_y, CV_32F, 0, 1, 3); // Y方向梯度
  cv::Sobel(cur_patch, cur_grad_x, CV_32F, 1, 0, 3); // X方向梯度
  cv::Sobel(cur_patch, cur_grad_y, CV_32F, 0, 1, 3); // Y方向梯度

  float error = 0.0f;
  for (int y = 0; y < ref_patch.rows; y++) {
      for (int x = 0; x < ref_patch.cols; x++) {
          // 计算梯度误差
          float dx_error = ref_grad_x.at<float>(y, x) - cur_grad_x.at<float>(y, x);
          float dy_error = ref_grad_y.at<float>(y, x) - cur_grad_y.at<float>(y, x);

          // 累加梯度误差的平方
          error += dx_error * dx_error + dy_error * dy_error;
      }
  }
  return error;
}
...
// zzh 优化后的Huber核函数
float huber_loss(float x, float delta = 1.0) {
  delta = std::max(1e-6f, delta); // 防止delta过小导致数值不稳定
  float abs_x = std::abs(x);
  if (abs_x <= delta) {
      return 0.5f * x * x;
  } else {
      return delta * (abs_x - 0.5f * delta);
  }
}
// zzh 优化后的Huber核函数
...
V3F VIOManager::getInterpolatedPixel(cv::Mat img, V2D pc)
{
  ...
  //       uint16_t *img_ptr = (uint16_t *)img.data + (v_ref_i * width + u_ref_i);
  //       float intensity = w_ref_tl * img_ptr[0] + w_ref_tr * img_ptr[1] + w_ref_bl * img_ptr[width] + w_ref_br * img_ptr[width + 1];
  //       pixel = V3F(intensity, intensity, intensity); // 单通道值赋值给RGB
  //   }
  //   else
  //   {
  //       uint8_t *img_ptr = (uint8_t *)img.data + ((v_ref_i)*width + (u_ref_i)) * 3;
